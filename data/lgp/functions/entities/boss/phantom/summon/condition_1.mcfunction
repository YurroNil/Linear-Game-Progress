...
execute if score phantomSummon.spawnChance lgp.world matches 1 at @r[limit=1,tag=!player.operTags,scores={lgp.PosY=45..}] positioned ~ ~20 ~ unless entity @e[tag=lgp.phantom] unless score phantomSummon.consecutiveTimes lgp.world matches 3.. summon minecraft:phantom run function lgp:entities/boss/phantom/summon/model_init
