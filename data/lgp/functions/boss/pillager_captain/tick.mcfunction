#设置BOSS
    execute if score pillagerCaptain.allowSummon lgp.world matches 1 if entity @a[scores={lgp.worldProcess=1..}] unless score inRaiders lgp.raiders matches 1 at @a unless entity @e[predicate=lgp:is_pillager_captain,tag=lgp.pillagerCaptain] as @e[tag=!lgp.pillagerCaptain,limit=1,predicate=lgp:is_pillager_captain] run function lgp:boss/pillager_captain/model
...
#设置掠夺者队长的生成CD为10天一周期
    ...
    execute if score #pillagerCaptain.inCD.dayPeriod lgp.world matches 10.. run function lgp:boss/pillager_captain/cd_reset
