#设置BOSS
    execute as @e[type=minecraft:witch,predicate=lgp:in_structure/swamp_hut,tag=!lgp.strangeWitch] run function lgp:boss/strange_witch/model
...
execute as @e[tag=lgp.strangeWitch] unless entity @s[scores={lgp.strangeWitch=220..}] at @s if entity @e[tag=player.group,tag=!player.operTags,distance=..7] run scoreboard players add @s lgp.strangeWitch 1
    ...
    #即将放技能-效果
    execute at @e[tag=lgp.strangeWitch,scores={lgp.strangeWitch=220}] run function lgp:boss/strange_witch/magic_prepare
    execute at @e[tag=lgp.strangeWitch,scores={lgp.strangeWitch=220..280}] run particle witch ~ ~ ~ 0.6 0.6 0.6 0 10 force @a
    #施法+女巫眩晕
    execute as @e[tag=lgp.strangeWitch,scores={lgp.strangeWitch=280}] at @s run function lgp:boss/strange_witch/magic_damage
    execute as @e[tag=lgp.strangeWitch,scores={lgp.strangeWitch=280..}] run item replace entity @s weapon.mainhand with air

    #女巫眩晕结束
    execute as @e[tag=lgp.strangeWitch,scores={lgp.strangeWitch=480..}] at @s run function lgp:boss/strange_witch/reset
