#设置生成BOSS
    ...
    execute unless score timeTick lgp.world matches 1..12999 run function lgp:boss/phantom/summon/tick
#条件2：杀死恐怖之翼或条件1满足完成三晚刷新后，会进入20天恐怖之翼的不刷新的周期。20天结束后回到条件1
    ...
    execute if score timeTick lgp.world matches 1 if score phantomSummon.unspawnPeriod lgp.world matches 21.. run function lgp:boss/phantom/summon/load
...
#技能
    ...
    #计分板
    execute if entity @e[tag=lgp.phantom] run function lgp:boss/phantom/install
    execute unless entity @e[tag=lgp.phantom] run function lgp:boss/phantom/uninstall
    execute as @e[tag=lgp.phantom] store result score @s lgp.phantom.health run data get entity @s Health 1.0

    #离玩家太远传送
        execute unless score timeTick lgp.world matches 1..12999 as @e[tag=lgp.phantom,tag=!lgp.phantom.sprinting] at @s unless entity @a[scores={lgp.PosY=50..},tag=!player.operTags,distance=..50] at @p[scores={lgp.PosY=50..},tag=!player.operTags] run function lgp:boss/phantom/tp
        #卡墙
        ...
        #白天自动"消失"
        execute if score timeTick lgp.world matches 1..12999 as @e[tag=lgp.phantom,tag=!lgp.phantom.sprinting] at @s run function lgp:boss/phantom/leave
    ...
    execute as @e[tag=lgp.phantom] at @s run function lgp:boss/phantom/sprint/loop_judge
...
#三阶段生怪
    ...
    execute as @e[tag=lgp.phantom,scores={lgp.phantom.summonSkill=467..}] at @s run function lgp:boss/phantom/summon_submobs
