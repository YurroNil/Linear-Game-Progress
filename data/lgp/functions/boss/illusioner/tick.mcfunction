#林地府邸BOSS生成
    ...
    #生成位置
    execute as @e[type=evoker,tag=!lgp.evoker,predicate=lgp:in_structure/mansion] at @s unless entity @e[type=illusioner,distance=..100] run tag @e[type=evoker,tag=!lgp.evoker,predicate=lgp:in_structure/mansion,limit=1] add lgp.illusioner.summonMarker
    #生成幻术师BOSS
    execute at @e[tag=lgp.illusioner.summonMarker] run function lgp:boss/illusioner/model

    #reset
    tag @e[tag=lgp.illusioner.summonMarker] remove lgp.illusioner.summonMarker
    tag @e[type=evoker,predicate=lgp:in_structure/mansion] add lgp.evoker
